...
if (isset($_POST['add_product'])) {
  ...
  $orderStatus = 0;
  $weight = $_POST['weight'];
  $weight = filter_var($weight, FILTER_SANITIZE_FULL_SPECIAL_CHARS);
  $color = $_POST['color'];
  $color = filter_var($color, FILTER_SANITIZE_FULL_SPECIAL_CHARS);
  $qty = $_POST['count'];
  $qty = filter_var($qty, FILTER_SANITIZE_FULL_SPECIAL_CHARS);
  
  $select_products = $conn->prepare("SELECT * FROM `orders` WHERE productId = ? AND userId = $user_id AND orderStatus = 0 AND colors = ? AND weight = ?");
  $select_products->execute([$id_product, $color, $weight]);
  $result = $select_products->fetch(PDO::FETCH_ASSOC);
  $qtyPartial = $result['quantity'];
  $qtyTotal = $qtyPartial + $qty;
  if ($select_products->rowCount() > 0) {
    $update_product = $conn->prepare("UPDATE `orders` SET quantity= ? WHERE productId = ? AND colors = ? AND weight = ?");
    $update_product->execute([$qtyTotal, $id_product, $color, $weight]);
  } else {

    $insert_products = $conn->prepare("INSERT INTO `orders`(userId, productId, quantity , orderStatus, colors, weight, invoiced) VALUES(?,?,?,?,?,?,?)");
          ...
          <div class="details_contain">
            <div class="details_whislist">
              <h2 class="details_h2"><?= $fetch["productName"] ?></h2>
                <?php if(!isset($user_id) || empty($user_id)){?><a class="details_span <?php if($status == 1){ echo "activado"; }?>" href="wishlist-add.php?id=<?= $fetch['id']?>"><svg class="details_svg" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <button id="agregar" name="add_product" type="submit" onclick="return validateForm()"><span><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="details_bi bi-plus" viewBox="0 0 16 16">
